...
class MasterMerger:
    ...
    def generate_timeline(self):
        ...
        timeline_config = self.config.get("timeline", {})
        output_file = timeline_config.get("output_file", "Policy4900_Approval_Timeline.xlsx")
        timeline_path = self.master_file.parent / output_file
        ...
        for csv_path in all_processed:
            try:
                report_date = self.extract_date_from_filename(csv_path.name)
        ...
        if not timeline_data:
            logging.warning("No data found for timeline generation")
            return

        # Filter to only classrooms that reached 100% at least once
        filtered_data = {}
        for key, dates in timeline_data.items():
            # Check if any date has 100%
            if any(pct >= 100.0 for pct in dates.values()):
                filtered_data[key] = dates

        if not filtered_data:
            logging.info("No classrooms have reached 100% approval yet")
            return
...
def main():
    ...
    # Timeline generation - auto-runs after all reports processed
    if merger.config.get("timeline", {}).get("auto_generate", True):
        print("\n[TIMELINE] Generating approval timeline...")
        try:
            merger.generate_timeline()
            timeline_file = merger.master_file.parent / merger.config.get("timeline", {}).get("output_file",
                                                                                              "Policy4900_Approval_Timeline.xlsx")
            print(f"[OK] Timeline updated: {timeline_file.name}")
        except Exception as e:
